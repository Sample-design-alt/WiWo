...
class WindowAttention(nn.Module):
    def __init__(self, dim, heads, head_dim, shifted, window_size, relative_pos_embedding,wa_dim,wa,prob,mask):
        ...
        if self.shifted:
            ...
            self.left_mask = nn.Parameter(create_mask(window_size=window_size,displacement=displacement,MASK=mask),requires_grad=False)
    ...
    def forward(self, x):
        ...
        b,p,f,h = *x.shape,self.heads          # 32,128,96,3
        # b batch_size   p : patch number f: feature

        if self.wa:
            y = self.sigmoid(self.window_attention(self.avgpool(x).reshape(b,p)))
        ...
        if not self.prob:
            ...
            scores_top, index = compute_q_k(q,k)
            ...
            context = _get_initial_context(v, self.window_size)
            out = _update_context(context, v, scores_top, index)
...
def compute_q_k(q,k):
    ...
    scores_top, index = prob_QK(q, k, sample_k=U_part, n_top=u)
...
def _update_context(context_in, V, scores, index):
    B, H, P, L_V, D = V.shape
...
class SwinBlock(nn.Module):
    def __init__(self, dim, heads, head_dim, mlp_dim, shifted, window_size, relative_pos_embedding,wa_dim,wa,prob,mask):
        super().__init__()
...
class PatchMerging(nn.Module):
    def __init__(self,in_channels=1,out_channels=64,downscaling_factor=4,embedding_dim=1024):
...
class StageModule(nn.Module):
    def __init__(self, in_channels, hidden_dimension, layers, downscaling_factor, num_heads, head_dim, window_size,wa,prob,mask,
                 relative_pos_embedding,embedding_dim=1024,wa_dim=4096):
        super().__init__()
        assert layers % 2 == 0, 'Stage layers need to be divisible by 2 for regular and shifted block.'

        self.patch_partition = PatchMerging(in_channels=in_channels, out_channels=hidden_dimension,
                                            downscaling_factor=downscaling_factor,embedding_dim=embedding_dim)
...
class SwinTransformer(nn.Module):
    def __init__(self, *,t=300,down_dim=1024,embedding_dim=1024,hidden_dim, layers, heads, channels, num_classes=1000, head_dim=32, window_size=7,
                 downscaling_factors=(4, 2, 2, 2), relative_pos_embedding=True,wa,prob,mask):
        super().__init__()
        self.downsample = nn.Linear(t,down_dim)
        self.stage1 = StageModule(in_channels=channels, hidden_dimension=hidden_dim, layers=layers[0],
                                  downscaling_factor=downscaling_factors[0], num_heads=heads[0], head_dim=head_dim,
                                  window_size=window_size, relative_pos_embedding=relative_pos_embedding,embedding_dim=embedding_dim,wa_dim = down_dim//downscaling_factors[0],wa=wa,prob=prob,mask=mask)
